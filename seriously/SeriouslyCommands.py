...
def cfsum(args):
    ...
    return complex(
        math.fsum([x.real for x in cargs]), math.fsum([x.imag for x in cargs])
    )
...
def str_base_float(number, base, exp):
    ...
    if exp < -15 or (
        number == 0 and exp < 0
    ):  # 15 places after the dot should be good, right?
        return ""
    return (
        digit_to_char(int(number), base)
        + ("." if exp == 0 and number % 1 else "")
        + str_base_float((number % 1) * base, base, exp - 1)
    )
...
def int_base(number, base):
    return reduce(
        lambda x, y: x * base + y, [char_to_digit(char, base) for char in number], 0
    )
...
fn_table = {
    ...
    0x10: lambda x: x.push(10),
    0x11: lambda x: x.push(x.pop() * 10),
    0x12: lambda x: x.push(x.pop() * 3),
    0x13: lambda x: x.push(x.pop() * 4),
    ...
    0x9C: fn_fil_fn,
    0x9D: lambda x: x.push(
        [a + b for a, b in itertools.zip_longest(x.pop(), x.pop(), fillvalue=0)]
    ),
    ...
    0xAF: ins_top_fn,
    0xB0: filter_true_fn,
    0xB1: lambda x: x.push(
        (lambda y: sum([1 if gcd(i, y) == 1 else 0 for i in range(1, y + 1)]))(x.pop())
    ),
    0xB2: prime_count_fn,
    0xB3: dupe_all_fn,
    ...
    0xF9: cart_prod_fn,
    0xFB: lambda x: x.push(x.pop() ** 0.5),
    0xFC: pow_fn,
